...
def ensure_defaults(db):
    ...
    count = db.execute("SELECT COUNT(*) AS c FROM casinos").fetchone()["c"]
    if count == 0:
    ...
    count = db.execute("SELECT COUNT(*) AS c FROM faqs").fetchone()["c"]
    if count == 0:
...
def ensure_admin(db):
    ...
    print("Guarda estas credenciales y cámbialas después de entrar.\n")
...
def get_site_data():
    ...
    casinos = []
    casino_rows = db.execute("SELECT id, icon, name FROM casinos ORDER BY position ASC, id ASC").fetchall()
    ...
    settings.setdefault("wa_url", f"https://wa.me/{settings.get('phone', DEFAULT_SETTINGS['phone'])}")
    return settings, casinos, faqs, DEFAULT_TESTIMONIALS


def validate_text(value, max_length=300, allow_empty=False):
...
def validate_url(value):
    ...
    if parsed.scheme != "https" or not parsed.netloc:
        raise ValueError("Todos los enlaces deben usar https:// y tener dominio válido.")
    if len(value) > 500:
